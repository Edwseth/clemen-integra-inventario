...
<!-- Fila con Fecha (día/mes/año) y número de OC + “citar este número…” -->
<div class="row mt-12">
    ...
    <div class="box" style="width:48%;">
        <table>
            <tr>
                <th class="small">Orden de Compra</th>
            </tr>
            <tr>
                <td class="right">No. ${numero}</td>
            </tr>
            <tr>
                <td class="small right">Citar éste número en el documento de entrega de la mercancía.</td>
            </tr>
        </table>
    </div>
</div>
...
<div class="row mt-8">
    <div class="box" style="width:48%;">
      ...
      <#if proveedor.telefono?has_content>
        , Teléfono: ${proveedor.telefono}
      </#if>
        <#if proveedor.paginaWeb?has_content>
        &nbsp;Página Web: ${proveedor.paginaWeb}
    </#if>
    </span>
</div>
...
<div class="mt-8">
    <table>
        ...
        <tbody>
        <!-- iterar items -->
        <!-- usa tu motor de plantillas; si es String.format, reemplaza por tu bucle -->
        <!-- Ejemplo con pseudo-sintaxis Freemarker: -->
        <#list items as it>
        <tr>
            <td>${it.codigo}</td>
            <td>${it.descripcion}</td>
            <td>${it.fechaNecesidad!''}</td>
            <td>${it.udm!''}</td>
            <td class="right">${it.cantidad}</td>
            <td class="right">${it.precioUnitario}</td>
            <td class="right">${it.iva}</td>
            <td class="right">${it.icui}</td>
